...
class DraftGUI:
    ...
    def refresh_teams_display(self):
        ...
        # Store the current team selection to restore it after refresh
        current_team = self.team_var.get()
        self.selected_team_container = None

        # Store team colors in a dictionary for persistence
        if not hasattr(self, 'team_color_indices'):
            self.team_color_indices = {}

        self.team_containers = {}  # Store containers for selection highlighting
        ...
        for tid, tinfo in all_teams.items():
            ...
            team_container.pack(side=tk.TOP, fill=tk.X, anchor="nw", pady=self.padding, padx=self.padding)
            ...
            def on_enter(e, container=team_container):
            ...
            def on_leave(e, container=team_container):
                if container != self.selected_team_container:
                    container.config(highlightbackground=self.ui_config["teams_bg_color"])
                e.widget.config(cursor="")
            
            def on_click(e, tid=tid, container=team_container):
                self.on_team_box_clicked(tid, container)
                return "break"  # Prevent propagation
            
            # Apply to team_card and all its children recursively
            def make_clickable(widget, tid=tid, container=team_container):
                ...
                for child in widget.winfo_children():
                    ...
                    make_clickable(child, tid, container)
            
            # Apply to team_card and its children
            make_clickable(team_card)
    ...
    def _set_team_color(self, team_id, color_index):
        """Set a team's color and refresh the display"""
        self.team_color_indices[team_id] = color_index
        self.refresh_teams_display()

    def on_team_box_clicked(self, team_id: str, container=None):
        """Handle team selection from team box click"""
        self.team_var.set(team_id)
        
        # Update visual selection state
        if hasattr(self, 'team_containers'):
        ...
        # Trigger preview to update automatically when team is selected
        self.preview_slices()
